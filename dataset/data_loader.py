...
class SentinelDataset(Dataset):
    ...
    def __init__(
        ...
        if self.normalize:
            ...
            self.norm_transform = transforms.Normalize(
                ...
                + [0.5] * (self.num_bands - 3),  # Use 0.5 for non-RGB bands
    ...
    def __getitem__(self, idx):
        ...
        with rasterio.open(self.image_path) as src:
            # Read window
            image = src.read(window=((y_min, y_max), (x_min, x_max)))

        # Read mask patch
        with rasterio.open(self.mask_path) as src:
            mask = src.read(
                1, window=((y_min, y_max), (x_min, x_max))  # Read first band
            )
        ...
        if image.shape[1] < patch_size or image.shape[2] < patch_size:
            ...
            padded_image = torch.zeros(
                (self.num_bands, patch_size, patch_size), dtype=torch.float32
            )
